import { Button,Text, Heading, Stack, VStack } from "@chakra-ui/react";
    ...
    <Stack direction={["column", "row"]} h={"100vh"}>
      <VStack w={"full"}>
        ...
        <VStack
          ...
          overflowY={"auto"}
        >
          <Heading >Sample Video {vidt}</Heading>
          <Text>This is a sample video used for testing the data</Text>
        </VStack>
      </VStack>
      <VStack width={["full", "xl"]} alignItems={'stretch'} p={'8'} spacing={'8'} overflowY={'auto'}>
          {videosArr.map((items,index)=>(
            <Button variant={'ghost'} colorScheme={'purple'} onClick={()=>{setVideosrc(items);setVidt(index+1)}}>
              Lecture {index+1}
            </Button>
          ))}
      </VStack>
